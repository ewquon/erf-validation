...
class NaluWindStats(ABLStats):
  ...
  def __init__(self,dir_name):
    ...
    stress_file = glob.glob(dir_name+'/*reynoldsstresses.dat')[0]
    a = pd.read_csv(stress_file, sep='\s+', skiprows=1, names=['z', 'uu', 'uv', 'uw', 'vv', 'vw', 'ww', 'tke'])
    self.vel_var = {
      "u'u'" : a["uu"],
      "u'v'" : a["uv"],
      "u'w'" : a["uw"],
...
class GABLSData(ABLStats):
  ...
  def __init__(self, dir_name):
    ...
    if len(a[:,0]) != len(self.z):
        print('diff z coord len for variances')
        self.z_var = a[:,0]
    elif not np.allclose(a[:,0], self.z):
        print('diff z coords for variances')
        self.z_var = a[:,0]
    b = gabls.readdata( glob.glob(dir_name+'/*C9*dat')[0])
    if len(b[:,0]) != len(self.z):
        print('diff z coord len for fluxes')
        self.z_flux = b[:,0]
    elif not np.allclose(b[:,0], self.z):
        print('diff z coords for fluxes')
        self.z_flux = b[:,0]
    self.vel_var = {
      "u'u'" : a[:,1],
      "u'v'" : a[:,2],
